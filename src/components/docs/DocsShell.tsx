...
const UPDATES_TOC = [
  ...
  ...UPDATE_LOGS.map((l) => ({ id: l.id, text: l.title, level: 2 })),
  { id: "latest", text: "最近一次对照明细", level: 2 },
        ...
        <Link to="/" className="flex min-w-0 items-center gap-2">
          <span className="flex size-7 shrink-0 items-center justify-center rounded-sm border border-border bg-bg-elevated">
            <svg viewBox="0 0 24 24" className="size-4 text-accent" fill="currentColor" aria-hidden>
              <path d="M4 7.5 12 3l8 4.5v9L12 21l-8-4.5v-9Zm8 1.2L8.2 10.8 12 13l3.8-2.2L12 8.7Zm-6 3.3v4.2L10 18.5v-4.2L6 12Zm12 0-4 2.3v4.2l4-2.3V12Z" />
            </svg>
          </span>
          <span className="hidden truncate font-medium tracking-tight sm:inline">Daytona 手册</span>
        </Link>
