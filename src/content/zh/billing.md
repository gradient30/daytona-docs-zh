...
Daytona provides an overview of your organization's [wallet](#wallet) and [spending](#spending). Daytona uses a pay-as-you-go billing model where you are charged based on the resources reserved for your sandboxes. For information on resource quotas, rate limits, and tier-based access, see [limits](/docs/limits).

## 沙箱计费 {#sandbox-billing}

Sandboxes are billed for the resources reserved: **vCPU**, **RAM**, and **disk**, depending on the sandbox [lifecycle state](/docs/sandboxes#sandbox-lifecycle). The table below details which reserved resources and states are billed for [container sandboxes](/docs/sandboxes#create-sandboxes), [VM sandboxes](/docs/sandboxes#vm-sandboxes), and [GPU sandboxes](/docs/sandboxes#gpu-sandboxes).

| **State** | **vCPU** | **RAM** | **Disk** | **Description** |
| ------------------------------------------------------- | -------- | ------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Started | ✓ | ✓ | ✓ | Billed for all reserved resources. |
| Creating   Starting   Stopping   Pausing | ✓ | ✓ | ✓ | Billed the same as started. 资源 remain reserved during these transitions, so reduced billing applies once the sandbox reaches the stopped or paused state, not when the stop or pause operation begins. |
| Stopped | ✗ | ✗ | ✓ | Billed for reserved disk only. [**Ephemeral sandboxes**](/docs/sandboxes#ephemeral-sandboxes)  and [**GPU sandboxes**](/docs/sandboxes#gpu-sandboxes) are automatically deleted when stopped, so stopped billing does not apply to them. |
| Paused | ✗ | ✗ | ✓ | Billed for reserved disk only. The preserved memory state is not billed. Supported for [**VM sandboxes**](/docs/sandboxes#vm-sandboxes) only. |
| Archived | ✗ | ✗ | ✗ | Not billed. Supported for [**container sandboxes**](/docs/sandboxes#create-sandboxes) only. |
| Deleted | ✗ | ✗ | ✗ | Not billed. [**快照 created from sandbox**](/docs/snapshots#create-snapshot-from-sandbox) remain billed for storage. |

Disk billing and [disk quota](/docs/limits#disk-quota) are separate: a sandbox can be billed for reserved disk without counting against your organization's storage limit.
...
3. Click Redeem to redeem the coupon code
...
### Per-sandbox usage {#per-sandbox-usage}
...
## Cancellation & post-cancellation {#cancellation-post-cancellation}

> Refer to [Daytona Terms of Service ↗](https://www.daytona.io/terms-of-service) for more information.

When you delete your [organization](/docs/organizations), cancel your subscription, or disable billing, you remain responsible for any sandbox usage that occurred before your action.

### 扣费 after cancellation {#charges-after-cancellation}
...
> 英文原文：https://www.daytona.io/docs/en/billing
> 本站位置：`/docs/billing`
