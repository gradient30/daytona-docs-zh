# Brainbase Universal Harness API {#universal-harness-api}

把 Brainbase harness 接到沙箱。
...
#### Describing the agent (config.ts) {#describing-the-agent-config-ts}
...
#### Talking to the API (client.ts) {#talking-to-the-api-client-ts}

`client.ts` is a thin wrapper over the REST endpoints. Every call is an authenticated HTTPS request that sends your Brainbase key as a bearer token; `this.headers()` adds the `Authorization: Bearer ` header, and a `Content-Type` is set only when there is a body.
...
#### Streaming the turn (index.ts) {#streaming-the-turn-index-ts}
...
#### Continuing the conversation (index.ts) {#continuing-the-conversation-index-ts}
...
#### Reporting the result (index.ts) {#reporting-the-result-index-ts}
...
#### Rendering events (render.ts) {#rendering-events-render-ts}
...
> 英文原文：https://www.daytona.io/docs/en/guides/brainbase/universal-harness-api
> 本站位置：`/docs/guides/brainbase/universal-harness-api`
