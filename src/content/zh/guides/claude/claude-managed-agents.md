...
### 各部分如何拼起来 {#how-the-pieces-fit-together}
...
### 你要准备什么 {#what-you-set-up}

#### 参考 implementation {#reference-implementation}
...
#### On the Anthropic side {#on-the-anthropic-side}
...
#### On the Daytona side {#on-the-daytona-side}
...
### 运行编排器 {#running-the-orchestrator}

#### The orchestrator process {#the-orchestrator-process}
...
- Archive sandboxes when their session terminates. The filesystem stays in Daytona's cost-effective 对象存储 until the same 30-day window expires, then the sandbox is deleted.
...
#### The in-sandbox runner {#the-in-sandbox-runner}
...
#### Mounting session resources {#mounting-session-resources}
...
#### MCP servers and vaults {#mcp-servers-and-vaults}
...
- *Build your sandbox snapshot however you like.* From a Dockerfile or Daytona's [declarative builder](/docs/declarative-builder): whatever language versions, system packages, or in-house tools the agent needs goes in there.
...
> 英文原文：https://www.daytona.io/docs/en/guides/claude/claude-managed-agents
> 本站位置：`/docs/guides/claude/claude-managed-agents`
