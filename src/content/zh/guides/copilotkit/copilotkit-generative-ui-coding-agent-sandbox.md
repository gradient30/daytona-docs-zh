# CopilotKit 生成式 UI 编程智能体 {#copilotkit-generative-ui-coding-agent-sandbox}
...
Every tool call streams into the chat as generative UI: shell commands render as terminal cards, file edits as syntax-highlighted code, listings and grep results as structured cards, and any hosted process (dev server, static site, API) as a live `` directly in the message stream.
...
### 3. 理解核心组件 {#3-understanding-the-core-components}
...
#### CopilotKit Built-in Agent {#copilotkit-built-in-agent}
...
#### Single-endpoint routing {#single-endpoint-routing}
...
#### Generative UI via `useRenderTool` {#generative-ui-via-userendertool}
...
The agent exposes 11 tools, defined with `defineTool` from `@copilotkit/runtime/v2`. Each is a thin wrapper around a [Daytona TypeScript SDK](/docs/typescript-sdk) call:
...
#### Step 1: Imports and the system prompt {#step-1-imports-and-the-system-prompt}
...
#### Step 2: Tool definitions {#step-2-tool-definitions}
...
#### Step 3: Mount the agent on `/api/copilotkit` {#step-3-mount-the-agent-on-api-copilotkit}
...
#### Step 4: Wrap the app in `CopilotKitRoot` {#step-4-wrap-the-app-in-copilotkitroot}

`components/CopilotKitRoot.tsx` is a thin client component that wraps children in the v2 provider and points it at the runtime endpoint. The provider sets up the AG-UI event stream and gives `` (and any other v2 components nested under it) access to the runtime:
...
#### Step 5: Register tool renderers with `useRenderTool` {#step-5-register-tool-renderers-with-userendertool}
...
#### Step 6: Configure suggestions {#step-6-configure-suggestions}
...
- **Any hosted process embeds as an `` directly in the chat**, with no link-outs or screenshots.
...
> 英文原文：https://www.daytona.io/docs/en/guides/copilotkit/copilotkit-generative-ui-coding-agent-sandbox
> 本站位置：`/docs/guides/copilotkit/copilotkit-generative-ui-coding-agent-sandbox`
