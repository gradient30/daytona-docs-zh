# Cursor 自托管机器 {#cursor-self-hosted-machines}

Self-Hosted Pool worker 跑在 Daytona 沙箱。
...
#### Install the Cursor controller {#install-the-cursor-controller}
...
The spawn command sets the requested repository as the workspace `origin`, and the worker starts with `agent worker --mint-github-token --on-session-start `. When Cursor claims the worker, it mints a short-lived GitHub token for the run, and the checkout hook fetches the requested ref into the workspace. Do not add a GitHub personal access token to `.env`.

#### Build a Daytona snapshot {#build-a-daytona-snapshot}
...
Each active worker uses one Daytona sandbox. Check your organization's [tier limits](/docs/limits) before you set pool concurrency.

### 运行控制器 {#run-the-controller}
...
- [Daytona snapshots](/docs/snapshots)
- [Daytona documentation](https://www.daytona.io/docs/index.md)

> 英文原文：https://www.daytona.io/docs/en/guides/cursor/cursor-self-hosted-machines
> 本站位置：`/docs/guides/cursor/cursor-self-hosted-machines`
