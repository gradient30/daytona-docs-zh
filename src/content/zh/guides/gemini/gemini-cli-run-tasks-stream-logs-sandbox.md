# Gemini CLI 跑任务并流式日志 {#gemini-cli-run-tasks-stream-logs-sandbox}
...
Inside the sandbox, `env` now shows `GEMINI_API_KEY=dtn_secret_...`, yet the CLI still authenticates: it sends the key as the `x-goog-api-key` HTTPS request header to `generativelanguage.googleapis.com`, where the proxy swaps in the real value. Substitution happens only in HTTPS request headers toward allowed hosts - requests to any other host carry the harmless placeholder. See the [Secrets documentation](/docs/secrets#substitution-scope) for the full substitution scope.
...
1. Creates a new [Daytona sandbox](/docs/sandboxes) with the Gemini API key injected as an environment variable.
...
#### Running Gemini Commands {#running-gemini-commands}
...
#### Streaming JSON 消息 {#streaming-json-messages}
...
> 英文原文：https://www.daytona.io/docs/en/guides/gemini/gemini-cli-run-tasks-stream-logs-sandbox
> 本站位置：`/docs/guides/gemini/gemini-cli-run-tasks-stream-logs-sandbox`
