# Google ADK 代码生成器 {#google-adk-code-generator}
...
#### Google ADK Components {#google-adk-components}
...
> There are two ways to run Google ADK agents: using the `App` class with `InMemoryRunner`, or using `InMemoryRunner` directly with just an agent. The `App` serves as a configuration container that bundles agents with plugins, while the `Runner` handles actual execution and 生命周期管理. This guide uses the `App` approach for cleaner organization of agents and plugins.

#### Daytona Plugin {#daytona-plugin}
...
### 4. Initialize Environment and Imports {#4-initialize-environment-and-imports}
...
### 5. Define the Response Extractor {#5-define-the-response-extractor}
...
### 6. Define the Agent Instruction {#6-define-the-agent-instruction}
...
### 7. 配置 Daytona 插件 {#7-configure-the-daytona-plugin}
...
### 9. 创建 App 与 Runner {#9-create-the-app-and-runner}
...
**What happens here:**
1. The `App` bundles the agent with the plugin for proper 生命周期管理
...
### 10. 运行示例 {#10-running-the-example}
...
#### 理解智能体执行流 {#understanding-the-agent-s-execution-flow}
...
#### Requesting Tests in the Response {#requesting-tests-in-the-response}
...
### 11. 完整实现 {#11-complete-implementation}
...
> 英文原文：https://www.daytona.io/docs/en/guides/google-adk/google-adk-code-generator
> 本站位置：`/docs/guides/google-adk/google-adk-code-generator`
