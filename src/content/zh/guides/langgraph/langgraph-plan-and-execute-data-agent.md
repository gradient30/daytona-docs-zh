# LangGraph 计划-执行数据智能体 {#langgraph-plan-and-execute-data-agent}

LangGraph agent + Daytona。
...
#### `StateGraph` with custom state {#stategraph-with-custom-state}
...
#### How nodes update state {#how-nodes-update-state}
...
#### 结构d-output planning {#structured-output-planning}
...
#### Step 1: State, schema, and prompts {#step-1-state-schema-and-prompts}
...
#### Step 2: The `provision` and `plan` nodes {#step-2-the-provision-and-plan-nodes}
...
#### Step 3: The `execute` node {#step-3-the-execute-node}
...
#### Step 4: The `check` node and conditional routing {#step-4-the-check-node-and-conditional-routing}
...
#### Step 5: `summarize` and `cleanup` {#step-5-summarize-and-cleanup}
...
#### Step 6: Wire the graph {#step-6-wire-the-graph}
...
#### Step 7: The `main()` entry point {#step-7-the-main-entry-point}
...
#### What the retry path catches {#what-the-retry-path-catches}
...
### 6. Adapting the Example {#6-adapting-the-example}
...
> 英文原文：https://www.daytona.io/docs/en/guides/langgraph/langgraph-plan-and-execute-data-agent
> 本站位置：`/docs/guides/langgraph/langgraph-plan-and-execute-data-agent`
