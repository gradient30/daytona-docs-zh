# Letta Code 智能体 {#letta-code-agent}
...
The agent can host web apps and provide you with a preview link using the [Daytona Preview Links](/docs/preview) feature. When your task involves running or previewing a web application, the agent automatically hosts the app and generates a link for you to inspect the live result:
...
Inside the sandbox, `env` now shows `LETTA_API_KEY=dtn_secret_...`, yet Letta Code still authenticates: it sends the key as an HTTPS `Authorization` header to `api.letta.com`, where the proxy swaps in the real value. Substitution happens only in HTTPS request headers toward allowed hosts - requests to any other host carry the harmless placeholder. See the [Secrets documentation](/docs/secrets#substitution-scope) for the full substitution scope.
...
### 3. 理解智能体架构 {#3-understanding-the-agent-s-architecture}
...
1. Creates a new [Daytona sandbox](/docs/sandboxes) with your Letta API key included in the environment variables.
2. Installs Letta Code globally inside the sandbox by running `npm install` with [process execution](/docs/process-code-execution#command-execution).
3. Creates a [PTY (pseudoterminal)](/docs/pty) session in the sandbox for bidirectional communication with Letta Code.
...
The program creates a [Daytona sandbox](/docs/sandboxes) with the Letta API key passed as an environment variable:
...
#### Running Letta Code in a Pseudoterminal {#running-letta-code-in-a-pseudoterminal}

A [PTY (pseudoterminal)](/docs/pty) is created for bidirectional communication with Letta Code:
...
> 英文原文：https://www.daytona.io/docs/en/guides/letta-code/letta-code-agent
> 本站位置：`/docs/guides/letta-code/letta-code-agent`
