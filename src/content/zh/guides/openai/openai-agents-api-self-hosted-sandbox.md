# OpenAI Agents API 自托管沙箱 {#openai-agents-api-self-hosted-sandbox}

Agents API 自托管到 Daytona。
...
Unlike the [OpenAI Agents SDK](/docs/guides/openai/openai-agents-sdk-with-sandboxes), where you host the agent loop yourself, the Agents API runs the harness as a managed service. You provide only the sandbox. The sandbox runs `codex exec-server`, which dials out to OpenAI and registers as the session's execution environment — no inbound ports are exposed.
...
When you create a self-hosted session you receive an environment ID and a session-specific `remote_url`. Whatever machine runs `codex exec-server --remote  --environment-id ` becomes that session's environment.
...
- **`OPENAI_WEBHOOK_SECRET`** — webhook-managed only. The signing secret for your OpenAI webhook endpoint, used to verify deliveries in the webhook handler.
...
The webhook-managed example additionally needs FastAPI and Uvicorn (`pip install fastapi uvicorn`).

### 3. Understanding the Integration {#3-understanding-the-integration}
...
#### Start the sandbox and executor {#start-the-sandbox-and-executor}
...
#### Why `auto_stop_interval=0` {#why-auto-stop-interval-0}
...
#### Send input and stream {#send-input-and-stream}
...
### 4. Application-managed vs Webhook-managed {#4-application-managed-vs-webhook-managed}
...
The companion `webhook_managed.py` implements the core of that handler — `connect_worker(...)` creates or wakes the sandbox and relaunches the executor, `stop_worker(...)` stops it on `agent.session.idle` to release compute, and `delete_worker(...)` tears it down — ready to wire into your own webhook endpoint. See OpenAI's [Daytona provider guide](https://developers.openai.com/api/docs/guides/agents-api/environments/providers/daytona) and [environment lifecycle](https://developers.openai.com/api/docs/guides/agents-api/environments/lifecycle) for the full webhook-managed flow.

### 5. Faster Connect with a Snapshot {#5-faster-connect-with-a-snapshot}

Building the image on first use keeps this example self-contained. For regular use, bake Codex and ripgrep into a [Daytona snapshot](/docs/snapshots) so the sandbox connects sooner instead of installing packages on every run.

### 6. A Note on Daytona Secrets {#6-a-note-on-daytona-secrets}

Other guides inject the OpenAI key with [Daytona Secrets](/docs/secrets), which keep the raw value out of the sandbox and substitute it into outbound HTTPS request headers. That works for tools that call `https://api.openai.com` directly. It is **not** currently suitable for the Agents API executor: its data plane is a WebSocket to `codex-cloud-environments.chatgpt.com`, and Secret substitution on that upgraded connection is not yet supported. Pass the executor key as an environment variable and keep it least-privilege (`api.agents.environments.connect`) so its exposure to sandbox code is harmless.
...
> 英文原文：https://www.daytona.io/docs/en/guides/openai/openai-agents-api-self-hosted-sandbox
> 本站位置：`/docs/guides/openai/openai-agents-api-self-hosted-sandbox`
