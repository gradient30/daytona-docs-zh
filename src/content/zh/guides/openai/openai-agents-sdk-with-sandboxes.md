# OpenAI Agents SDK 与沙箱 {#openai-agents-sdk-with-sandboxes}

Agents SDK 直接创建 Daytona 沙箱。
...
See also the [Text-to-SQL Agent with the OpenAI Agents SDK and Daytona](/docs/guides/openai/text-to-sql-agent-openai-agents-sdk) guide for a complete project built on these patterns.
...
## 1\. Give Your Agent a Shell {#1-give-your-agent-a-shell}
...
## 3\. Pause and Resume {#3-pause-and-resume}
...
## 4\. Handoffs: Routing Work Between Agents {#4-handoffs-routing-work-between-agents}
...
## 5\. Sandbox Agents as Tools {#5-sandbox-agents-as-tools}
...
Each sandbox agent runs in its own 隔离环境. The orchestrator never sees the files; it only gets the structured output as JSON via the `custom_output_extractor`. This is great for **fan-out** patterns where you need multiple independent analyses.

## 6\. Memory Across Sessions {#6-memory-across-sessions}
...
## 7\. Custom Capabilities {#7-custom-capabilities}
...
## Using a Different Model Provider {#using-a-different-model-provider}
...
## Quick 参考: DaytonaSandboxClientOptions {#quick-reference-daytonasandboxclientoptions}

| Option | Default | Description |
| :---- | :---- | :---- |
| `image` | `None` | OCI-compliant image to boot from |
| `env_vars` | `None` | 环境变量 injected at creation |
...
For a complete project that puts these patterns to work, see [**Building a Text-to-SQL Agent with OpenAI Agents SDK and Daytona**](/docs/guides/openai/text-to-sql-agent-openai-agents-sdk), a conversational agent that queries real federal spending data, combining multi-turn conversations, pause/resume, memory, and preview URLs.

> 英文原文：https://www.daytona.io/docs/en/guides/openai/openai-agents-sdk-with-sandboxes
> 本站位置：`/docs/guides/openai/openai-agents-sdk-with-sandboxes`
