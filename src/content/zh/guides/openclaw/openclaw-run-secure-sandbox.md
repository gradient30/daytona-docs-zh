# OpenClaw 安全沙箱 {#openclaw-run-secure-sandbox}
...
> Already have the CLI? Check your version with `daytona --version` and keep it current — older versions miss newer sandbox commands. [Upgrade to the latest version](/docs/tools/cli).
...
### Run OpenClaw Onboarding {#run-openclaw-onboarding}
...
### Allow Dashboard Access Through the Preview URL {#allow-dashboard-access-through-the-preview-url}
...
This generates a [signed preview URL](/docs/preview#signed-preview-url) that securely exposes the port. You will open it in the browser later.
...
The OpenClaw dashboard is a 网页界面 for managing your assistant, monitoring connections, and configuring channels.

Open the [preview URL](/docs/preview) you generated earlier in your browser, and paste your gateway token when the Control UI prompts for it (the value you printed after onboarding).
...
### Pair Your Browser {#pair-your-browser}
...
#### Install the WhatsApp plugin {#install-the-whatsapp-plugin}
...
#### Link WhatsApp (QR) {#link-whatsapp-qr}
...
> 英文原文：https://www.daytona.io/docs/en/guides/openclaw/openclaw-run-secure-sandbox
> 本站位置：`/docs/guides/openclaw/openclaw-run-secure-sandbox`
