# OpenClaw SDK 沙箱 {#openclaw-sdk-sandbox}

OpenClaw SDK + Daytona。

This guide shows how to run [OpenClaw](https://openclaw.ai/) inside a Daytona sandbox using the Daytona SDK. The script automatically creates and configures a sandbox with OpenClaw and provides an authenticated [preview URL](/docs/preview) for using OpenClaw in the browser.
...
You can use the Control UI to chat with your assistant, configure Telegram and WhatsApp, and manage sessions. When you exit the script (Ctrl+C), the sandbox will not be deleted unless [sandbox persistence is disabled](/docs/guides/openclaw/openclaw-sdk-sandbox#4-key-constants).
...
1. The script creates a [Daytona sandbox](/docs/sandboxes) with `DAYTONA_SNAPSHOT` (e.g. `daytona-medium`) and loads env vars from `.env.sandbox`. The sandbox is private unless you set `MAKE_PUBLIC`.
2. A [preview link](/docs/preview) for `LOCAL_PROXY_PORT` is resolved first, because its origin has to be allowlisted in the gateway config before the gateway starts.
...
4. The OpenClaw gateway is started on `OPENCLAW_PORT` via [process execution](/docs/process-code-execution), and the script waits until it responds (retrying the start if needed).
...
> 英文原文：https://www.daytona.io/docs/en/guides/openclaw/openclaw-sdk-sandbox
> 本站位置：`/docs/guides/openclaw/openclaw-sdk-sandbox`
