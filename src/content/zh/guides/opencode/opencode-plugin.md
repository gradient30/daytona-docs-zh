# OpenCode 插件 {#opencode-plugin}

OpenCode 插件对接 Daytona。
...
### 3. Understanding the Plugin Architecture {#3-understanding-the-plugin-architecture}

The Daytona plugin consists of several modules that provide custom tools, hooks for events, connections from sessions to sandboxes and system prompt transforms. These modules ensure every agent action runs in a [Daytona sandbox](/docs/sandboxes) and that changes sync to local Git branches:
...
All stateful tools (bash, read, write, edit, glob, grep, ls, lsp, multiedit, patch) are overridden the same way. The plugin also adds a custom tool for [preview links](/docs/preview).

#### System prompt transform {#system-prompt-transform}
...
- Live [preview links](/docs/preview) when a server starts in the sandbox
...
> 英文原文：https://www.daytona.io/docs/en/guides/opencode/opencode-plugin
> 本站位置：`/docs/guides/opencode/opencode-plugin`
