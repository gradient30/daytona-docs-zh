...
### 1. OpenEnv and Daytona {#1-openenv-and-daytona}
...
OpenEnv has a pluggable provider model for _where_ environments run: local Docker, Docker Swarm, or Daytona. The `DaytonaProvider` launches each environment instance as a Daytona sandbox, which gives you API-driven 生命周期管理 and the ability to run thousands of instances in parallel without local infrastructure. In the training loop below, each sandbox is an independent FinQA environment with its own persistent WebSocket connection.

### 2. 工作流总览 {#2-workflow-overview}
...
### 4. Running a Single Episode {#4-running-a-single-episode}
...
### 5. Understanding the Training Code {#5-understanding-the-training-code}
...
#### Tool Schema Fetching {#tool-schema-fetching}
...
#### Sandbox Pool 管理 {#sandbox-pool-management}
...
#### Multi-Turn Rollout Collection {#multi-turn-rollout-collection}
...
#### Stale WebSocket 清理 After Cancellation {#stale-websocket-cleanup-after-cancellation}
...
#### GRPO: Grouping and Advantages {#grpo-grouping-and-advantages}
...
#### GRPO Policy Gradient Update {#grpo-policy-gradient-update}
...
#### GPU Layout and Lag-1 Pipeline {#gpu-layout-and-lag-1-pipeline}
...
### 6. Running the Training {#6-running-the-training}
...
#### What Did the Model Actually Learn? {#what-did-the-model-actually-learn}
...
#### The Year-Column Quoting Fix {#the-year-column-quoting-fix}
...
#### Numeric Answer Formatting {#numeric-answer-formatting}
...
#### Parenthetical Negative Notation {#parenthetical-negative-notation}
...
#### Adaptive Error Recovery {#adaptive-error-recovery}
...
#### Shortcut Arithmetic {#shortcut-arithmetic}
...
### 8. Training 配置 {#8-training-configuration}
...
- **Safe execution**: SQL queries and data exploration execute in 隔离环境s
- **OpenEnv protocol**: Standard RL environment interface over WebSocket, decoupling the environment from the agent

> 英文原文：https://www.daytona.io/docs/en/guides/reinforcement-learning/openenv-finqa
> 本站位置：`/docs/guides/reinforcement-learning/openenv-finqa`
