...
[Daytona Limits ↗](https://app.daytona.io/dashboard/limits) provide an overview of your organization's [resources](#resources), [sandbox limits](#sandbox-limits), and [rate limits](#rate-limits). Daytona uses a [tier-based](#tiers) system where organizations are placed into tiers based on verification status, with each tier providing access to a specific compute pool and rate limits. For information on spending and wallet management, see [billing](/docs/billing).
...
Disk quota and [sandbox billing](/docs/billing#sandbox-billing) are separate: a sandbox can be billed for reserved disk without counting against your organization's storage limit. The table below details which states occupy disk quota for [container sandboxes](/docs/sandboxes#create-sandboxes) and [VM sandboxes](/docs/sandboxes#vm-sandboxes).

| **State** | **Container Sandbox** | **VM Sandbox   (Linux VM and Windows)** | **Description** |
| --------- | --------------------- | -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Paused | ✗ | ✗ | Paused VM sandboxes free the disk quota. Supported for [**VM sandboxes**](/docs/sandboxes#vm-sandboxes) only. |
| Archived | ✗ | ✗ | [**Archiving**](/docs/sandboxes#archive-sandboxes) moves the container filesystem to 对象存储, frees the quota, and stops billing. Supported for [**container sandboxes**](/docs/sandboxes#create-sandboxes) only. VM sandboxes have no archive state because stopping or pausing them already frees the quota. |
...
Daytona [Python](https://www.daytona.io/docs/python-sdk.md), [TypeScript](/docs/typescript-sdk), [Ruby](https://www.daytona.io/docs/ruby-sdk.md) and [Go](https://www.daytona.io/docs/go-sdk.md) SDKs raise or throw a `DaytonaRateLimitError` exception (Python) or error (TypeScript, Ruby and Go) when you exceed a rate limit.
...
| **Tier** | **资源 (vCPU / RAM / Storage)** | **Access Requirements** |
| -------- | ------------------------------------ | ------------------------------------------------------- |
...
> [Network limits](/docs/network-limits) are automatically applied based on your organization's billing tier.
