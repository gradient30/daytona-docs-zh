...
- **Pre-built snapshot** — build a [snapshot](/docs/snapshots) once with the FUSE tool (`mount-s3`, `gcsfuse`, `blobfuse2`, `box-mount`) built-in, then launch every sandbox from that snapshot. Cold starts are fast and predictable. Best for production.
...
## Mount an Amazon S3 bucket {#mount-an-amazon-s3-bucket}
...
#### Launch and mount {#launch-and-mount}
...
## Mount a Cloudflare R2 bucket {#mount-a-cloudflare-r2-bucket}
...
## Mount a Tigris bucket {#mount-a-tigris-bucket}
...
### Mount a copy-on-write fork per sandbox {#mount-a-copy-on-write-fork-per-sandbox}
...
#### 参考: Tigris-specific headers {#reference-tigris-specific-headers}
...
## Mount a Google Cloud Storage bucket {#mount-a-google-cloud-storage-bucket}
...
## Mount an Azure Blob container {#mount-an-azure-blob-container}
...
image = (
  ...
  .run_commands(
    ...
    # libfuse3.so.3 compat symlink for Trixie (see >     'src=$(find /usr/lib /lib -name "libfuse3.so.3.*" -type f 2>/dev/null '
    "| sort -V | tail -1) "
...
## Mount a Box folder {#mount-a-box-folder}
...
### Install Box Mount {#install-box-mount}
...
public class App {
    public static void main(String[] args) {
        try (Daytona daytona = new Daytona()) {
            ...
            daytona.snapshot().create("box-mount", image, System.out::println);
        }
    }
}
```
...
### Mount with JWT server authentication {#mount-with-jwt-server-authentication}
...
### Mount with OAuth {#mount-with-oauth}
...
## Mount an Archil disk {#mount-an-archil-disk}

[Archil ↗](https://archil.com) is an infinite, elastic, POSIX file system that automatically synchronizes to 对象存储 like S3, R2, and Azure Blob. You should use Archil when you need to mount a bucket to your Daytona sandbox, but need higher out-of-the-box performance than traditional FUSE mounts. Archil achieves higher performance by using shared SSD read and write caching in front of your 对象存储 bucket. Archil disks are mounted as regular directories, scale to whatever your sandbox writes (you pay only for what you use), and can be [mounted by many sandboxes at once ↗](https://docs.archil.com/concepts/shared-disks), making them a natural fit for parallel agents that share state.
...
### Shared and read-only mounts {#shared-and-read-only-mounts}
...
- **Egress allowlists break Archil.** A sandbox with a [`domainAllowList`](/docs/network-limits) cannot mount Archil disks: the Archil control plane is reached as a bare IP address on port `8100`, which a domain allowlist cannot express. The installer also downloads from `s3.amazonaws.com/archil-client/`, which must be reachable at install time. A [`networkAllowList`](/docs/network-limits) (CIDR) can pin Archil's mount and control IP addresses, but those addresses change, so this is fragile and not recommended. Use full egress for sandboxes that mount Archil disks.
...
## Mount a MesaFS filesystem {#mount-a-mesafs-filesystem}
...
### Production: scoped ephemeral keys {#production-scoped-ephemeral-keys}
...
> 英文原文：https://www.daytona.io/docs/en/mount-external-storage
> 本站位置：`/docs/mount-external-storage`
