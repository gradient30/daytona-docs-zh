# 进程与代码执行 {#process-code-execution}
...
- [Run code (stateless)](#run-code-stateless): run independent code snippets where each execution starts from a clean interpreter state; inherits the sandbox language that you choose at [sandbox creation](/docs/sandboxes#create-sandboxes). Supports Python, JavaScript, and TypeScript.
...
### Get session command {#get-session-command}
...
### Execute interactive commands {#execute-interactive-commands}
...
> 英文原文：https://www.daytona.io/docs/en/process-code-execution
> 本站位置：`/docs/process-code-execution`
