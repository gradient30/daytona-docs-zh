...
| ------------------------ | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Sandbox (vertical) | vCPU, RAM, and disk of a single sandbox | • [**Custom resources**](/docs/sandboxes#resources)   • [**Resize**](/docs/sandboxes#resize-sandboxes) |
| Fleet (horizontal) | Number of sandboxes running at the same time | • [**Snapshot fan-out**](/docs/snapshots)   • [**Fork**](/docs/sandboxes#fork-sandboxes)   • [**链接沙箱**](/docs/sandboxes#linked-sandboxes) |
| Workload (concurrency) | Concurrent processes inside a single sandbox | • [**Sessions**](/docs/process-code-execution#session-operations)   • [**Async commands**](/docs/process-code-execution#session-operations)   • [**PTY sessions**](/docs/pty) |

## 沙箱扩展 {#sandbox-scaling}
...
Change the 资源分配 of an existing sandbox.
...
| --------------------------------------------------------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| [**Snapshot fan-out**](/docs/snapshots) | Filesystem from the snapshot; hot snapshots add memory state | Any number of identical environments from one prepared snapshot |
| [**Fork**](/docs/sandboxes#fork-sandboxes) | Filesystem and memory of a running VM sandbox | Branch a live environment into independent copies |
| [**链接沙箱**](/docs/sandboxes#linked-sandboxes) | Fresh environment, co-located with a parent on the same runner | Coordinated groups with a local network between parent and children |

**Snapshot fan-out** is the default pattern: prepare the environment once, capture it as a snapshot, and create any number of sandboxes from it. Each sandbox starts with the environment intact, with nothing to reinstall. Sandboxes created from a [hot snapshot](/docs/snapshots#create-snapshot-from-sandbox) start with processes already running.
...
**Forking** duplicates a running VM sandbox, filesystem and memory included, into an independent sandbox. Where snapshot fan-out distributes a prepared baseline, forks branch live state: each fork continues from the exact point the original was at. Forking is supported for [VM sandboxes](/docs/sandboxes#vm-sandboxes) only.
...
## 工作负载并发 {#workload-concurrency}

Workload concurrency runs multiple processes inside a single sandbox. A single sandbox can run an API server, a database, and background workers side by side: all processes share the sandbox's filesystem and network, and exposed ports are reachable through [previews](/docs/preview).

A sandbox runs concurrent processes through [sessions](/docs/process-code-execution#session-operations). Each session is an independent shell with its own state: working directory, environment variables, and command history. 

Commands within a session run sequentially, so parallelism comes from running multiple sessions. Commands started with `run_async` return immediately and run in the background, and [PTY sessions](/docs/pty) add interactive terminals.
...
## Capacity and throughput {#capacity-and-throughput}

Fleet size and creation rate operate within your organization's limits. Both are [tier-based](/docs/limits#tiers) and raised by verification steps or by contacting support.

| **Control** | **What it limits** | **Scope** |
| ------------------------------------------------------------------ | ----------------------------------------------------------------------- | -------------------------------------------------- |
| [**Compute pool**](/docs/limits#resources) | Total vCPU, RAM, and storage across all running sandboxes | Organization, per region and sandbox class |
| [**Per-sandbox limits**](/docs/limits) | Maximum vCPU, RAM, and storage of a single sandbox | 沙箱 |
| [**Rate limits**](/docs/limits#rate-limits) | Sandbox creation, lifecycle operations, and API requests per minute | 组织 |

To scale beyond the shared compute pool, [bring your own compute (BYOC)](/docs/bring-your-own-compute) attaches your own runner nodes in custom regions, which have no concurrent resource usage limits.

### Reclaiming capacity {#reclaiming-capacity}
...
Reclamation runs automatically. [Ephemeral sandboxes](/docs/sandboxes#ephemeral-sandboxes) delete themselves on stop, and the [auto-stop, auto-pause, and auto-delete intervals](/docs/sandboxes#automated-lifecycle-management) reclaim capacity from idle sandboxes without manual intervention. See [persistence](/docs/persistence#retention-and-lifecycle) for details.

### Operating at scale {#operating-at-scale}

At high creation rates, two practices keep an application within its rate limits.

- Handle [rate limit errors](/docs/limits#rate-limit-errors): the SDKs raise **`DaytonaRateLimitError`** when a limit is exceeded, and the returned headers indicate how long to wait before retrying the request.
- Use [webhooks](/docs/webhooks) to track sandbox state changes instead of polling. Webhooks deliver state changes as they happen, so no requests are spent checking for status.

> 英文原文：https://www.daytona.io/docs/en/scale
> 本站位置：`/docs/scale`
