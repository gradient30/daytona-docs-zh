...
Interactive reference: /docs/tools/api
...
- `PATCH /organizations/{organizationId}/identity-providers/{id}` - Update organization 身份提供商
- `DELETE /organizations/{organizationId}/identity-providers/{id}` - Delete organization 身份提供商
- `POST /organizations/{organizationId}/identity-providers/test-connection` - Test OIDC 身份提供商 connection
- `GET /organizations/{organizationId}/gpu-capacity` - Get shared GPU capacity

### users {#users}

- `GET /users/me` - Get authenticated user
- `POST /users/me/logins` - Record a completed login
- `POST /users/privacy-policies/accept` - Accept the current privacy policies
- `GET /users/account-providers` - Get account providers
...
## Daytona Toolbox API {#daytona-toolbox-api}
...
## Daytona Analytics API {#daytona-analytics-api}
...
> 英文原文：https://www.daytona.io/docs/en/tools/api
> 本站位置：`/docs/tools/api`
