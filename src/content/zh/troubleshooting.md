...
A sandbox moves from create through start and into later size and placement changes. Create selects a [sandbox class](/docs/sandboxes) and reserves resources from the regional pool. [Provisioning](#provisioning) then decides how that create is fulfilled: snapshot builds, [warm pools](/docs/warm-pools), [region](/docs/regions) availability, and [resize](/docs/sandboxes#resize-sandboxes).

### 创建与启动 {#create-and-start}

Create and start allocate a sandbox from a snapshot or image and bring it to a running state. The request selects a [sandbox class](/docs/sandboxes) and reserves **vCPU**, **memory**, and **disk** from the organization's regional pool. Each class has its own create rules and quota.
...
3. [Archive](/docs/sandboxes#archive-sandboxes) or delete stopped container sandboxes to free disk quota
4. Upgrade your [tier](/docs/limits#tiers) if the pool itself is too small

See [create sandboxes](/docs/sandboxes#create-sandboxes) and [disk quota](/docs/limits#disk-quota).
...
**Cause:** Linux VM sandboxes can only be created from existing [VM snapshots](/docs/snapshots#vm-snapshots) such as `daytona-vm-small`, or a custom snapshot with sandbox class `LINUX_VM`. The declarative builder applies to [container sandboxes](/docs/sandboxes#create-sandboxes) only. Quotas are tracked per sandbox class and region, so unused container capacity does not free Linux VM capacity.
...
See [VM sandboxes](/docs/sandboxes#vm-sandboxes) and [VM snapshots](/docs/snapshots#vm-snapshots).
...
**Cause:** Windows sandboxes can only be created from existing [VM snapshots](/docs/snapshots#vm-snapshots) such as `windows-small`, or a custom snapshot with sandbox class `WINDOWS`. The declarative builder applies to [container sandboxes](/docs/sandboxes#create-sandboxes) only. Quotas are tracked per sandbox class and region, so unused container capacity does not free Windows capacity.
...
**Symptom:** Creating a GPU sandbox fails with `400` / `DaytonaBadRequestError`. The message typically reports that the total GPU limit is exceeded, sometimes with `Maximum allowed: 0`. A GPU sandbox may also stay in the `Pending Build` state instead of starting. Inside a started GPU sandbox, CUDA may also fail with device-unavailable errors such as error `999`.

**Cause:** GPU sandboxes on shared regions run in the [Earth region](/docs/regions#earth-region): Daytona selects the shared region and ignores the requested target, and error messages report `earth` as the region. The organization either has no GPU quota in the Earth region, or existing and pending GPU sandboxes already use the available quota. A GPU sandbox in `Pending Build` is waiting for GPU capacity of the requested type to become available. A started sandbox can also land on a GPU that is not healthy for CUDA workloads.

**Solution:**

1. Confirm your organization has GPU quota under the Earth region in [Limits ↗](https://app.daytona.io/dashboard/limits)
2. Check [GPU capacity](/docs/sandboxes#gpu-capacity) for the requested type. Fall back to another GPU type or create a [spot GPU sandbox](/docs/sandboxes#spot-gpu-sandboxes) if capacity is available
3. Try again later if pending GPU usage is consuming the pool
4. If CUDA fails inside a started sandbox, delete it and create a new GPU sandbox
5. For a specific region or repeated device faults, contact [support@daytona.io](mailto:support@daytona.io)

See [GPU sandboxes](/docs/sandboxes#gpu-sandboxes) and [regions](/docs/regions).

### 供给 {#provisioning}

Provisioning covers how a create is fulfilled after the sandbox class is chosen: [warm pools](/docs/warm-pools), [snapshot](/docs/snapshots) builds, [region](/docs/regions) availability, and [resize](/docs/sandboxes#resize-sandboxes) of reserved resources.
...
**Solution:** Align the create request with the pool, or accept a cold create when you need custom env, volumes, secrets, or resources. If the API returns `404`, use the Dashboard **Warm Pool** controls when available, or contact [support@daytona.io](mailto:support@daytona.io) to enable warm pools for your organization. See [warm pools](/docs/warm-pools).
...
**Solution:** Stop the sandbox, resize within [organization limits](/docs/limits), then start. See [resize sandboxes](/docs/sandboxes#resize-sandboxes).
...
See [recover sandboxes](/docs/sandboxes#recover-sandboxes) and [snapshots](/docs/snapshots).
...
3. For SDKs, CLI, or API calls, use an [API key](/docs/api-keys) created in that same organization

See [organizations](/docs/organizations) and [personal vs collaborative](/docs/organizations#personal-vs-collaborative).
...
State covers whether a started sandbox stays usable: [recovery](/docs/sandboxes#recover-sandboxes) from error, [automated lifecycle](/docs/sandboxes#automated-lifecycle-management) stops and deletes, and operations that depend on the [sandbox class](/docs/sandboxes).
...
3. If it is not recoverable, delete the sandbox and create a new one from a [snapshot](/docs/snapshots)

See [recover sandboxes](/docs/sandboxes#recover-sandboxes).
...
**Cause:** [Auto-stop](/docs/sandboxes#auto-stop-interval) or [auto-pause](/docs/sandboxes#auto-pause-interval) treats the sandbox as idle when there is no qualifying activity. Processes that only run inside the sandbox do not count. Activity that resets the timer includes toolbox requests, preview traffic, SSH sessions, and lifecycle state changes.
...
See [automated 生命周期管理](/docs/sandboxes#automated-lifecycle-management).
...
**Cause:** The sandbox reached a lifetime bound and was removed. That happens when it was created as [ephemeral](/docs/sandboxes#ephemeral-sandboxes) and then stopped, when [auto-delete](/docs/sandboxes#auto-delete-interval) elapsed after it stayed stopped, or when [wall-clock TTL](/docs/sandboxes#wall-clock-ttl) (`ttl_minutes`) expired in any state.

**Solution:** Check the create parameters and any later `set_ttl` / `set_auto_delete_interval` calls. Persist results to a [volume](/docs/volumes) or [snapshot](/docs/snapshots) before the sandbox is destroyed.
...
**Cause:** Those operations are available only on [VM sandboxes](/docs/sandboxes#vm-sandboxes) (Linux VM or Windows). Container and GPU sandboxes preserve the filesystem across stop and start, but they do not support pause, fork, or hot snapshots.

**Solution:** Create from a VM snapshot such as `daytona-vm-small`, or use stop/start and a [cold snapshot](/docs/snapshots#create-snapshot-from-sandbox) for filesystem-only capture.

### 访问 {#access}

Access covers reaching services inside a started sandbox and driving work through it: [preview](/docs/preview), [VNC](/docs/vnc-access), [network](/docs/network-limits) policy, [process](/docs/process-code-execution) execution, and [rate limits](/docs/limits#rate-limits).
...
**Solution:** Use the matching auth method for the URL type. Refresh standard tokens after restart. Check signed URL expiry. See [preview](/docs/preview).
...
**Cause:** VNC and Computer Use need the desktop stack in the sandbox image. Custom images without the [required packages](/docs/vnc-access#required-packages) report that Computer Use is not available. The sandbox must also be started. Intermittent `503` or failed sessions usually clear after restarting Computer Use or recreating the sandbox.

**Solution:**

1. Confirm the sandbox is started
2. Use a default image, or install the packages listed in [VNC access](/docs/vnc-access#required-packages)
...
See [isolation](/docs/isolation#network-isolation) and [create sandboxes with network restrictions](/docs/network-limits#create-sandboxes-with-network-restrictions).
...
**Solution:** Raise the timeout, or run long jobs asynchronously in a [session](/docs/process-code-execution#session-operations) with `run_async=True` and poll for completion. For ended sessions or completed commands, create a new session and read the prior exit code or logs instead of re-executing.
...
**Cause:** The organization exceeded a rate limit window for general API requests, sandbox creation, or sandbox lifecycle operations. Sandbox creation has its own per-minute cap by [tier](/docs/limits#rate-limits). Bursting above that cap, for example creating hundreds of sandboxes in one minute, returns `429` until the window resets.

**Solution:** Read `Retry-After-{throttler}` from the error headers, wait, then retry with backoff. Pace concurrent creates to stay under the sandbox-create limit, or contact [support@daytona.io](mailto:support@daytona.io) for a higher quota. See [rate limit retry](#rate-limit-retry) and [rate limit errors](/docs/limits#rate-limit-errors).
...
3. Confirm the key has the [permissions](/docs/api-keys#permissions--scopes) required for the call
...
See [API keys](/docs/api-keys) and [organizations](/docs/organizations).
...
| **`401`** | **`Daytona认证Error`** | Missing or invalid credentials | Refresh API key / auth; do not retry with the same key |
...
### Daemon error codes {#daemon-error-codes}

| **Code** | **SDK class** | **HTTP class** | **Meaning** |
| -------------------------------- | ------------------------------------------ | -------------------------------- | -------------------------------------------------------- |
| **`GIT_AUTH_FAILED`** | **`DaytonaGitAuthFailedError`** | 认证 (`401`) | Git credentials rejected by the remote |
...
| **`INVALID_FILE_PATH`** | **`DaytonaInvalidFilePathError`** | Bad request (`400`) | Invalid filesystem path (输入Script SDK) |
| **`FILE_READ_FAILED`** | **`DaytonaFileReadFailedError`** | Internal server (`500`) | Filesystem read failed (输入Script SDK) |
...
> **Full reference**: [Python SDK errors](/docs/python-sdk/common/errors), [TypeScript SDK errors](/docs/typescript-sdk/errors), [Go SDK errors](/docs/go-sdk/errors).
...
| **`DaytonaInvalidArgumentError`** | SDK rejected arguments locally before sending a request (输入Script) | Fix arguments; do not retry as-is |
