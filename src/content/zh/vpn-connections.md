...
> For connecting to a VPN network, you need to [create or access your Daytona sandbox](/docs/sandboxes), **have access to your VPN network provider credentials**, and be on [**Tier 3** or higher](/docs/limits#resources).
...
### Tailscale 浏览器登录 {#tailscale-browser-login}
...
### Tailscale 授权密钥 {#tailscale-auth-key}
...
### Tailscale web terminal {#tailscale-web-terminal}

For working directly in the terminal or for more control over the Tailscale connection process, you can set up Tailscale manually through the Daytona [web terminal](/docs/web-terminal) or [SSH](/docs/ssh-access).
...
1. Navigate to your sandbox [web terminal](/docs/web-terminal) in [Daytona Dashboard ↗](https://app.daytona.io/), or [access it via SSH](/docs/ssh-access)
...
### OpenVPN client configuration file {#openvpn-client-configuration-file}
...
# OpenVPN client configuration (paste your .ovpn config here) {#openvpn-client-configuration-paste-your-ovpn-config-here}
OPENVPN_CONFIG = """
""".strip()

# Initialize the Daytona client {#initialize-the-daytona-client}
config = DaytonaConfig(api_key=DAYTONA_API_KEY)
daytona = Daytona(config)
...
Daytona provides a [web terminal](/docs/web-terminal) for interacting with your sandboxes, allowing you to install OpenVPN and connect to your OpenVPN network.

1. Navigate to your sandbox [web terminal](/docs/web-terminal) in [Daytona Dashboard ↗](https://app.daytona.io/) by clicking on the Terminal icon `>_`, or [access it via SSH](/docs/ssh-access)
...
> 英文原文：https://www.daytona.io/docs/en/vpn-connections
> 本站位置：`/docs/vpn-connections`
